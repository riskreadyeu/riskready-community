...
function registerRiskMutations(server: McpServer) {
  ...
  server.tool(
    ...
    'Propose creating a new Key Risk Indicator. Requires human approval. The reason field is shown to human reviewers. Only cite facts retrieved from tools.',
    {
      ...
      greenThreshold: z.string().max(200).optional().describe('Green threshold value'),
      amberThreshold: z.string().max(200).optional().describe('Amber threshold value'),
      redThreshold: z.string().max(200).optional().describe('Red threshold value'),
