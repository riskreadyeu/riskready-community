...
function registerAssetMutations(server: McpServer) {
  server.tool(
    ...
    {
      name: z.string().max(500).describe('Asset name'),
      ...
      assetSubtype: z.string().max(200).optional().describe('Asset subtype'),
      status: z.enum([
      ...
      cloudRegion: z.string().max(200).optional().describe('Cloud region'),
      ...
      scopeNotes: z.string().max(2000).optional().describe('Scope notes'),
      // Physical location
      ...
      lifecycleNotes: z.string().max(2000).optional().describe('Lifecycle notes'),
      // Technical
      ipAddresses: z.array(z.string()).max(100).optional().describe('IP addresses (JSON array)'),
      macAddresses: z.array(z.string()).max(100).optional().describe('MAC addresses (JSON array)'),
      operatingSystem: z.string().max(200).optional().describe('Operating system'),
      osVersion: z.string().max(200).optional().describe('OS version'),
      version: z.string().max(200).optional().describe('Software/firmware version'),
      patchLevel: z.string().max(200).optional().describe('Current patch level'),
      // Vendor
      manufacturer: z.string().max(200).optional().describe('Manufacturer'),
      ...
      encryptionMethod: z.string().max(200).optional().describe('Encryption method'),
      backupEnabled: z.boolean().optional().describe('Whether backups are enabled'),
      backupFrequency: z.string().max(200).optional().describe('Backup frequency'),
      backupRetention: z.string().max(200).optional().describe('Backup retention period'),
      ...
      // Capacity
      cpuCapacity: z.number().int().max(1_000_000).optional().describe('CPU capacity (cores)'),
      memoryCapacityGB: z.number().max(1_000_000).optional().describe('Memory capacity in GB'),
      storageCapacityGB: z.number().max(1_000_000).optional().describe('Storage capacity in GB'),
      networkBandwidthMbps: z.number().max(1_000_000).optional().describe('Network bandwidth in Mbps'),
      cpuThresholdPercent: z.number().max(100).optional().describe('CPU threshold percent'),
      memoryThresholdPercent: z.number().max(100).optional().describe('Memory threshold percent'),
      storageThresholdPercent: z.number().max(100).optional().describe('Storage threshold percent'),
      capacityNotes: z.string().max(2000).optional().describe('Capacity notes'),
      // Resilience
...
function registerChangeMutations(server: McpServer) {
  server.tool(
    ...
    {
      title: z.string().max(500).describe('Change request title'),
      description: z.string().max(5000).describe('Change request description'),
      ...
      businessJustification: z.string().max(5000).optional().describe('Business justification for the change'),
      ...
      affectedServices: z.array(z.string().max(200)).max(100).optional().describe('List of affected service names'),
  ...
  server.tool(
    ...
    'Propose updating a change request. Validates the change exists before creating the proposal. Requires human approval. The reason field is shown to human reviewers. Only cite facts retrieved from tools.',
    {
      ...
      businessJustification: z.string().max(5000).optional().describe('Business justification'),
      status: z.enum([
...
function registerCapacityMutations(server: McpServer) {
  server.tool(
    ...
    'Propose creating a new capacity plan for an asset. Supports NIS2 capacity management requirements. Requires human approval. The reason field is shown to human reviewers. Only cite facts retrieved from tools.',
    {
      ...
      recommendedAction: z.string().max(5000).optional().describe('Recommended action to address capacity'),
      estimatedCost: z.number().max(1_000_000_000).optional().describe('Estimated cost for the recommended action'),
  ...
  server.tool(
    ...
    'Propose updating a capacity plan. Validates the capacity plan exists before creating the proposal. Requires human approval. The reason field is shown to human reviewers. Only cite facts retrieved from tools.',
    {
      ...
      currentCapacity: z.string().max(5000).optional().describe('Current capacity description'),
      currentUtilizationPercent: z.number().int().min(0).max(100).optional().describe('Current utilization percentage'),
      projectedGrowthPercent: z.number().optional().describe('New projected growth percentage'),
      projectionPeriodMonths: z.number().int().optional().describe('Projection period in months'),
      projectedExhaustionDate: z.string().datetime().optional().describe('Projected capacity exhaustion date (ISO 8601)'),
      recommendedAction: z.string().max(5000).optional().describe('New recommended action'),
