    ...
    try {
      await prisma.riskScenario.upsert({
        ...
        update: {
          ...
          sleLow: row['SLE_Low'] ? parseFloat(row['SLE_Low']) : undefined,
          sleLikely: row['SLE_Likely'] ? parseFloat(row['SLE_Likely']) : undefined,
          sleHigh: row['SLE_High'] ? parseFloat(row['SLE_High']) : undefined,
          framework: mapFramework(String(row['Framework'] || 'ISO')),
