...
function registerAssessmentMutations(server: McpServer) {
  ...
  server.tool(
    ...
    'Propose bulk-assigning tests to testers, owners, or assessors. Requires human approval.',
    {
      ...
      testMethod: z.string().optional().describe('Test method to set'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
...
function registerSoaMutations(server: McpServer) {
  server.tool(
    ...
    'Propose updating an SOA entry — change applicability or implementation status. Requires human approval.',
    {
      ...
      implementationDesc: z.string().optional().describe('Implementation description'),
      justificationIfNa: z.string().optional().describe('Justification if marking not applicable'),
      ...
      scenarioIds: z.string().optional().describe('Comma-separated scenario IDs (e.g. "R-001-S01, R-001-S02")'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
  ...
  server.tool(
    ...
    'Propose creating a new empty Statement of Applicability. Requires human approval.',
    {
      ...
      notes: z.string().optional().describe('SOA notes'),
      organisationId: z.string().optional().describe('Organisation UUID (uses default if omitted)'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
  ...
  server.tool(
    ...
    'Propose creating a new SOA version by copying entries from an existing SOA. Requires human approval.',
    {
      ...
      notes: z.string().optional().describe('New SOA notes'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
...
function registerScopeMutations(server: McpServer) {
  server.tool(
    ...
    'Propose adding a new scope item (application, asset class, location, etc.) for assessment testing. Requires human approval.',
    {
      ...
      code: z.string().describe('Unique scope item code (e.g. "APP-001", "LOC-HQ")'),
      name: z.string().describe('Scope item name'),
...
function registerTestMutations(server: McpServer) {
  server.tool(
    ...
    'Propose recording a test result for an assessment test. Requires human approval.',
    {
      ...
      findings: z.string().optional().describe('Test findings'),
      recommendations: z.string().optional().describe('Recommendations'),
      testMethod: z.string().optional().describe('Test method used'),
      ...
      rootCauseNotes: z.string().optional().describe('Root cause notes'),
      ...
      estimatedCost: z.number().optional().describe('Estimated cost to remediate'),
  ...
  server.tool(
    ...
    'Propose a remediation action for a failed assessment test. Creates a treatment action linked to the test. Requires human approval.',
    {
      ...
      description: z.string().optional().describe('Detailed description of the remediation'),
      priority: z.enum(['CRITICAL', 'HIGH', 'MEDIUM', 'LOW']).default('MEDIUM').describe('Priority level'),
      estimatedHours: z.number().int().optional().describe('Estimated hours to complete'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
  ...
  server.tool(
    ...
    'Propose updating root cause analysis for a failed or partial test. Requires human approval.',
    {
      ...
      rootCauseNotes: z.string().optional().describe('Root cause notes'),
      remediationEffort: z.enum(['TRIVIAL', 'MINOR', 'MODERATE', 'MAJOR', 'STRATEGIC']).optional().describe('Remediation effort'),
      estimatedHours: z.number().optional().describe('Estimated hours to remediate'),
      estimatedCost: z.number().optional().describe('Estimated cost to remediate'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
...
function registerControlMutations(server: McpServer) {
  server.tool(
    ...
    'Propose changing a control\'s implementation status or applicability. Requires human approval.',
    {
      ...
      justificationIfNa: z.string().optional().describe('Justification when marking control as not applicable'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
  ...
  server.tool(
    ...
    'Propose creating a new control in the library. Requires human approval.',
    {
      ...
      sourceStandard: z.string().optional().describe('Source standard reference'),
      soc2Criteria: z.string().optional().describe('SOC2 criteria'),
      tscCategory: z.string().optional().describe('TSC category'),
      applicable: z.boolean().optional().describe('Whether control is applicable'),
      justificationIfNa: z.string().optional().describe('Justification if not applicable'),
  ...
  server.tool(
    ...
    'Propose updating an existing control\'s details. Requires human approval.',
    {
      ...
      sourceStandard: z.string().optional().describe('New source standard'),
      soc2Criteria: z.string().optional().describe('New SOC2 criteria'),
      tscCategory: z.string().optional().describe('New TSC category'),
      applicable: z.boolean().optional().describe('New applicability'),
      justificationIfNa: z.string().optional().describe('Justification if not applicable'),
      implementationStatus: z.enum(['NOT_STARTED', 'PARTIAL', 'IMPLEMENTED']).optional().describe('New implementation status'),
      implementationDesc: z.string().optional().describe('New implementation description'),
      reason: z.string().optional().describe('Explain WHY this change is proposed — shown to human reviewers'),
      mcpSessionId: z.string().optional().describe('MCP session identifier for tracking'),
    },
