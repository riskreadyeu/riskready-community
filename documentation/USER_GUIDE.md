...
View team member count, workspace configuration, and SSO status.
...
Configure the built-in AI assistant that powers intelligent analysis across the platform:

- **Anthropic API Key** -- Enter your Anthropic API key to enable AI features. The key is stored securely and displayed in masked form once set. You can replace or clear the key at any time.
- **Model Selection** -- Choose the Claude model for AI operations:
  - Claude Haiku 4.5 -- Fast and cost-effective for routine queries.
  - Claude Sonnet 4.5 -- Balanced performance for most use cases.
  - Claude Opus 4 -- Most capable for complex analysis.
- **Gateway URL** -- The URL of the AI gateway service (default: `http://localhost:3100`).
- **Max Agent Turns** -- Control how many tool-use turns the AI can take per request (1-100). Lower values are faster and cheaper; higher values allow more thorough analysis.

### Integrations

Connect RiskReady to external tools such as Jira, Slack, and GitHub.
