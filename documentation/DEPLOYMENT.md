...
# 3. Edit .env -- at minimum, set the REQUIRED variables listed below
...
Once the services are healthy, open **http://localhost:8080** in your browser and log in
with the `ADMIN_EMAIL` and `ADMIN_PASSWORD` values you configured in `.env`.
...
6. **caddy** (reverse proxy, ports 8080/8443) -- starts after both server and web are ready.
...
| `CORS_ORIGIN`                 | No       | `http://localhost:5173,http://localhost:8080`     | Comma-separated list of allowed CORS origins.              |
...
|-----------|----------------|------------|-----------------------------------------------------|
| 5433      | 5432           | db         | PostgreSQL -- exposed for local tooling only.       |
| 3000      | 3000           | server     | NestJS API.                                         |
| 3100      | 3100           | gateway    | AI Gateway.                                         |
| 8080      | 80             | caddy      | HTTP entry point (reverse proxy).                   |
| 8443      | 443            | caddy      | HTTPS entry point (reverse proxy).                  |
...
- Configure the load balancer to forward traffic to `http://<host>:8080`.
...
RiskReady Community Edition includes an AI gateway that connects to Anthropic's
Claude API through 8 MCP (Model Context Protocol) servers, exposing 248 tools for
GRC-specific intelligence.

To enable AI features after deployment:

1. Log in to the web interface at `http://localhost:8080`.
2. Navigate to **Settings > AI Configuration**.
3. Enter your Anthropic API key.
4. Save the configuration.

The gateway will begin processing requests using Claude once the API key is
configured. No restart is required.
...
Navigate to **http://localhost:8080** in your browser. The login page should load.

**5. Log in:**

Use the `ADMIN_EMAIL` and `ADMIN_PASSWORD` values from your `.env` file to
authenticate.
