...
The first open-source GRC platform with a built-in AI agent powered by the Model Context Protocol (MCP).
<br />
Query, analyse, and update your compliance data through natural language.
...
| **Approval Queue** | All mutations create pending actions reviewed at `/settings/mcp-approvals` |
...
The default admin account is created on first run using the `ADMIN_EMAIL` and `ADMIN_PASSWORD` values from your `.env` file.

To enable AI features, sign in and add your Anthropic API key at **Settings > AI Configuration**.
...
How the AI gateway works under the hood: setup, the 8 MCP servers and their 248 tools, example queries for each GRC domain, the human-in-the-loop approval queue, memory and context, channel integrations (Slack, Discord), using MCP servers standalone with Claude Desktop or Claude Code, anti-hallucination safeguards, and model selection.
